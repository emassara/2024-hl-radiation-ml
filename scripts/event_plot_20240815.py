...
def main():
    ...
    for date_start, date_end, file_name, title_prefix in plots_to_produce:
        ...
        fig, axs = plt.subplot_mosaic([['hmi_m', 'aia_0131', 'aia_0171', 'aia_0193', 'aia_0211', 'aia_1600'],
                                    ...
                                    ], figsize=(20, 10), height_ratios=[2, 1.5, 1.5])
        ...
        ax = axs['crater']
        # ax.set_title('GOES X-ray flux')
        ax.text(0.005, 0.96, 'CRaTER absorbed dose rate', ha='left', va='top', transform=ax.transAxes, fontsize=12)
        ax.set_ylabel('μGy/h')
        ax.yaxis.set_label_position("right")
        goes_dates, goes_values = crater.get_series(date_start, date_end, delta_minutes=args.delta_minutes)
        if goes_dates is not None:
            ax.plot(goes_dates, goes_values, color='green', alpha=0.75)
        ax.xaxis.set_minor_locator(hours_locator)
        ax.grid(color='#f0f0f0', zorder=0, which='minor', axis='x')
        ax.grid(color='lightgray', zorder=0, which='major')
        ax.set_xticks(axs['biosentinel'].get_xticks())
