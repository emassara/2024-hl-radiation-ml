...
def load_model(file_name, device):
    ...
    if checkpoint['model'] == 'RadRecurrent':    
        ...
        model = RadRecurrent(data_dim_context=model_data_dim_context, 
                            data_dim_predict=model_data_dim_prediction,
                            lstm_dim=model_lstm_dim, lstm_depth=model_lstm_depth, dropout=model_dropout, context_window=model_context_window, prediction_window=model_prediction_window)
...
def save_test_file(prediction_dates, goesxrs_predictions, rad_predictions, goesxrs_ground_truth_dates, goesxrs_ground_truth_values, rad_ground_truth_dates, rad_ground_truth_values, file_name):
    ...
    rad_prediction_mean = np.mean(rad_predictions, axis=0)
    rad_prediction_std = np.std(rad_predictions, axis=0)

    with open(file_name, 'w') as f:
        f.write('date,goesxrs_prediction_mean,goesxrs_prediction_std, rad_prediction_mean,rad_prediction_std,goesxrs_ground_truth,rad_ground_truth\n')
        for i in range(len(prediction_dates)):
            ...
            rad_prediction_mean_value = rad_prediction_mean[i]
            rad_prediction_std_value = rad_prediction_std[i]
            ...
            if date in rad_ground_truth_dates:
                rad_ground_truth_value = rad_ground_truth_values[rad_ground_truth_dates.index(date)]
            else:
                rad_ground_truth_value = float('nan')
            ...
            f.write('{},{},{},{},{}\n'.format(date, #goessgps10_prediction_mean_value, goessgps10_prediction_std_value, goessgps100_prediction_mean_value, goessgps100_prediction_std_value, 
                                            ...
                                            rad_prediction_mean_value, rad_prediction_std_value, #goessgps10_ground_truth_value, goessgps100_ground_truth_value, 
                                            goesxrs_ground_truth_value, rad_ground_truth_value))
            

def save_test_plot(context_dates, prediction_dates, training_prediction_window_end, goesxrs_predictions, rad_predictions, goesxrs_ground_truth_dates, goesxrs_ground_truth_values, rad_ground_truth_dates, rad_ground_truth_values, file_name, args, title=None):
    print('Saving test plot: {}'.format(file_name))
    fig, axs = plt.subplot_mosaic([['rad'],['goesxrs']], figsize=(20, 10), height_ratios=[1,1])

    num_samples = rad_predictions.shape[0]

    ylims = {}
    hours_locator = matplotlib.dates.HourLocator(interval=1)
    colors = {}
    colors['rad'] = 'mediumblue'
    ...
    ax.plot(rad_ground_truth_dates, rad_ground_truth_values, color=colors['rad'], label='Ground truth', alpha=0.75)
    ax.plot(prediction_dates, np.mean(rad_predictions, axis=0), color=colors['prediction'], alpha=prediction_mean_alpha, label='Prediction (mean)')
    for i in range(num_samples):
        label = 'Prediction (samples)' if i == 0 else None
        ax.plot(prediction_dates, rad_predictions[i], label=label, color=colors['prediction'], alpha=prediction_alpha)
    ...
    # ax.set_xticklabels([])
    # ax.set_yscale('log')
    # ax.set_xticks(axs['rad'].get_xticks())
    # ax.set_xlim(axs['rad'].get_xlim())
...
def run_test(model, main_study_dir, dir_test_plot, date_start, date_end, file_prefix, title, args):
    ...
    if isinstance(model, RadRecurrentWithSDO):
        # dataset_sdo = SDOMLlite(data_dir_sdo, main_study_dir, date_start=context_start, date_end=date_end)
        dataset_sdo = SDOMLlite(data_dir_sdo, data_dir_sdo, date_start=context_start, date_end=date_end)
    ...
    if isinstance(model, RadRecurrent):
        ...
        print('context_sdo_batch', context_sdo_batch.shape)
        print('context_data_batch', context_data_batch.shape)
    ...
    rad_predictions = prediction_batch[:, :, 1]
    ...
    rad_predictions = dataset_rad.unnormalize_data(rad_predictions).cpu().numpy()
    ...
    rad_ground_truth_dates, rad_ground_truth_values = dataset_rad.get_series(context_start, date_end, delta_minutes=args.delta_minutes)
    ...
    rad_ground_truth_values = dataset_rad.unnormalize_data(rad_ground_truth_values)
    ...
    save_test_file(prediction_dates, #goessgps10_predictions, goessgps100_predictions, 
                   goesxrs_predictions, 
                    rad_predictions, #goessgps10_ground_truth_dates, goessgps10_ground_truth_values, goessgps100_ground_truth_dates, goessgps100_ground_truth_values, 
                    goesxrs_ground_truth_dates, goesxrs_ground_truth_values, rad_ground_truth_dates, rad_ground_truth_values, test_file)
    ...
    ylims = save_test_plot(context_dates, prediction_dates, training_prediction_window_end, #goessgps10_predictions, goessgps100_predictions, 
                           goesxrs_predictions, 
                        rad_predictions, #goessgps10_ground_truth_dates, goessgps10_ground_truth_values, goessgps100_ground_truth_dates, goessgps100_ground_truth_values, 
                        goesxrs_ground_truth_dates, goesxrs_ground_truth_values, rad_ground_truth_dates, rad_ground_truth_values, test_plot_file, args, title=title)
    return ylims
...
def run_test_video(model, main_study_dir, dir_test_plot, date_start, date_end, file_prefix, title_prefix, ylims, args):
    ...
    if isinstance(model, RadRecurrentWithSDO):
        # dataset_sdo = SDOMLlite(data_dir_sdo, main_study_dir, date_start=full_start, date_end=date_end)
        dataset_sdo = SDOMLlite(data_dir_sdo, data_dir_sdo, date_start=full_start, date_end=date_end)
    ...
    rad_ground_truth_dates, rad_ground_truth_values = dataset_rad.get_series(full_start, full_end, delta_minutes=args.delta_minutes)
    ...
    if isinstance(model, RadRecurrentWithSDO):
        ...
        fig, axs = plt.subplot_mosaic([['rad', 'rad', 'rad', 'rad'],
    ...
    colors['goesxrs'] = 'purple'
    colors['prediction'] = 'red'
    colors['prediction_mean'] = 'brown' #'brown' #'darkred'
    colors['time'] = 'black'

    prediction_alpha = 0.15 #0.08
    prediction_mean_alpha = 1 #0.55
    prediction_secondary_alpha = prediction_alpha * 0.5
    prediction_secondary_mean_alpha =prediction_mean_alpha * 0.5
    prediction_mean_lw = 2
    ...
    ax.yaxis.set_label_position("right")
    ax.plot(rad_ground_truth_dates, rad_ground_truth_values, color=colors['rad'], alpha=0.75, label='Ground truth')
    ax.xaxis.set_minor_locator(hours_locator)
    ax.grid(color='#f0f0f0', zorder=0, which='minor', axis='x')
    ax.grid(color='lightgray', zorder=0, which='major')
    ax.set_xticklabels([])
    ...
    ims['rad_context_start'] = ax.axvline(context_start, color=colors['time'], linestyle='--', linewidth=1.5) # Context start
    ims['rad_prediction_start'] = ax.axvline(prediction_start, color=colors['time'], linestyle='-', linewidth=1.5) # Context end / Prediction start
    ims['rad_training_prediction_end'] = ax.axvline(training_prediction_end, color=colors['time'], linestyle='--', linewidth=1.5) # Prediction end
    # ims['rad_now_text'] = ax.text(prediction_start + datetime.timedelta(minutes=5), ylims['rad'][0], 'Now',verticalalignment='bottom', horizontalalignment='left')
    # prediction plots
    ims['rad_prediction_mean'] = ax.plot([], [], color=colors['prediction_mean'], alpha=prediction_mean_alpha, label='Prediction (mean)',lw=prediction_mean_lw)[0]
    # ims['rad_prediction_std_upper'] = ax.plot([], [], color=colors['prediction'], alpha=prediction_mean_alpha, label='Prediction (std dev)')[0]
    # ims['rad_prediction_std_lower'] = ax.plot([], [], color=colors['prediction'], alpha=prediction_mean_alpha)[0]
    ims['rad_prediction_mean_secondary'] = ax.plot([], [], color=colors['prediction_mean'], alpha=prediction_secondary_mean_alpha,lw=prediction_mean_lw)[0]
    # ims['rad_prediction_std_secondary_upper'] = ax.plot([], [], color=colors['prediction'], alpha=prediction_secondary_mean_alpha)[0]
    # ims['rad_prediction_std_secondary_lower'] = ax.plot([], [], color=colors['prediction'], alpha=prediction_secondary_mean_alpha)[0]
    for i in range(args.num_samples):
        label = 'Prediction (samples)' if i == 0 else None
        ims['rad_prediction_{}'.format(i)], = ax.plot([], [], color=colors['prediction'], alpha=prediction_alpha, label=label)
        ims['rad_prediction_{}_secondary'.format(i)], = ax.plot([], [], color=colors['prediction'], alpha=prediction_secondary_alpha)
    ax.legend(loc='upper right')
    ax.set_ylim(ylims['rad'])
    ...
    # ax.xaxis.set_major_locator(plt.MaxNLocator(num_ticks))
    ims['goesxrs_context_start'] = ax.axvline(context_start, color=colors['time'], linestyle='--', linewidth=1.5) # Context start
    ims['goesxrs_prediction_start'] = ax.axvline(prediction_start, color=colors['time'], linestyle='-', linewidth=1.5) # Context end / Prediction start
    ims['goesxrs_training_prediction_end'] = ax.axvline(training_prediction_end, color=colors['time'], linestyle='--', linewidth=1.5) # Prediction end
    ...
    # prediction plots
    ims['goesxrs_prediction_mean'] = ax.plot([], [], color=colors['prediction_mean'], alpha=prediction_mean_alpha, lw=prediction_mean_lw)[0]
    ...
    ims['goesxrs_prediction_mean_secondary'] = ax.plot([], [], color=colors['prediction_mean'], alpha=prediction_secondary_mean_alpha, lw=prediction_mean_lw)[0]
    ...
    with tqdm(total=num_frames) as pbar:
        def run(i):
            ...
            ims['rad_context_start'].set_xdata([context_start_date, context_start_date])
            ims['rad_prediction_start'].set_xdata([prediction_start_date, prediction_start_date])
            ims['rad_training_prediction_end'].set_xdata([training_prediction_end_date, training_prediction_end_date])
            # ims['rad_now_text'].set_position((prediction_start_date + datetime.timedelta(minutes=5), ylims['rad'][0]))
            ...
            rad_predictions_primary = rad_predictions[:, :model.prediction_window+1]
            rad_predictions_secondary = rad_predictions[:, model.prediction_window:]
            ...
            ims['rad_prediction_mean'].set_data(prediction_dates_primary, np.mean(rad_predictions_primary, axis=0))
            ...
            # ims['rad_prediction_std_upper'].set_data(prediction_dates_primary, np.mean(rad_predictions_primary, axis=0) + np.std(rad_predictions_primary, axis=0))
            # ims['rad_prediction_std_lower'].set_data(prediction_dates_primary, np.mean(rad_predictions_primary, axis=0) - np.std(rad_predictions_primary, axis=0))
            ...
            ims['rad_prediction_mean_secondary'].set_data(prediction_dates_secondary, np.mean(rad_predictions_secondary, axis=0))
            ...
            # ims['rad_prediction_std_secondary_upper'].set_data(prediction_dates_secondary, np.mean(rad_predictions_secondary, axis=0) + np.std(rad_predictions_secondary, axis=0))
            # ims['rad_prediction_std_secondary_lower'].set_data(prediction_dates_secondary, np.mean(rad_predictions_secondary, axis=0) - np.std(rad_predictions_secondary, axis=0))
            ...
            for i in range(args.num_samples):
                ims['rad_prediction_{}'.format(i)].set_data(prediction_dates_primary, rad_predictions_primary[i])
                ...
                ims['rad_prediction_{}_secondary'.format(i)].set_data(prediction_dates_secondary, rad_predictions_secondary[i])
    ...
    with tqdm(total=num_frames) as pbar:
        for i in range(num_frames):
            ...
            if not prediction_start_date.day == current_now_day.day:
                ...
                np.save(file_rad,rad_p)

                file_xray = dir_test_pred +'/{}-xray_pred-{}-{}wprediction.npy'.format(event_prefix,current_now_day.strftime('%Y%m%d'),args.multiples_prediction_window)
                ...
                rad_t = np.array(rad_t)
                np.save(file_rad_truth,rad_t)
            ...
            rad_predictions_primary = rad_predictions[:, :model.prediction_window+1]
            rad_predictions_secondary = rad_predictions[:, model.prediction_window+1:]
            goessgps10_predictions_primary = goessgps10_predictions[:, :model.prediction_window+1]
            ...
            for i_date in prediction_dates:
                ...
                if i_date in rad_ground_truth_dates:
                    rad_array.append(rad_ground_truth_values[rad_ground_truth_dates.index(i_date)])
                else:
                    rad_array.append(float('nan'))
            rad_array = np.array(rad_array)
            goesxrs_array = np.array(goesxrs_array)
            rad_t.append(rad_array)
...
def main():
    ...
    parser.add_argument('--solar_dataset', type=str, choices=['SDOMLlite', 'SDOCore', None], default='SDOMLlite', help='Solar dataset type')
    ...
    parser.add_argument('--input_datasets', nargs='+', default=['sdo', 'goes_xrs', 'rad'], help='What datasets to use in input')
    parser.add_argument('--output_datasets', nargs='+', default=['goes_xrs', 'rad'], help='What datasets to use in output')

    parser.add_argument('--model_file', type=str, default=None, help='Model file')
    parser.add_argument('--multiples_prediction_window', type=int, default=1, help='how many multiples of the prediction_window to save for autoregressive predictions')
    ...
    print(f"input datasets: {args.input_datasets}")
    print(f"output datasets: {args.output_datasets}")

    # make sure the target directory exists
    os.makedirs(args.target_dir, exist_ok=True)

    ## Create the ablation study run directory
    main_study_dir = args.target_dir+f"/input-datasets--{','.join(args.input_datasets)}/output_datasets--{','.join(args.output_datasets)}/solar-data--{args.solar_dataset}/rad-inst--{args.rad_inst}"
    ...
    with Tee(log_file):
        ...
        if args.mode == 'train':
            ...
            ## Rutuja's version of the code
            print('Processing excluded dates')
            if 'sdo' in args.input_datasets:
                datasets_sdo_valid = []
                datasets_sdo_test = []
            if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                datasets_goes_xrs_valid = []
                datasets_goes_xrs_test = []
            if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                datasets_rad_valid = []
                datasets_rad_test = []
            ...
            if args.valid_event_id is not None:
                for event_id in args.valid_event_id:
                    ...
                    if 'sdo' in args.input_datasets and args.solar_dataset == 'SDOMLlite':
                        datasets_sdo_valid.append(SDOMLlite(data_dir_sdo, data_dir_sdo, date_start=exclusion_start, date_end=exclusion_end))
                    if 'sdo' in args.input_datasets and args.solar_dataset == 'SDOCore':
                        datasets_sdo_valid.append(SDOCore(data_dir_sdo, date_start=exclusion_start, date_end=exclusion_end))
                    if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                    ...
                    if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                        datasets_rad_valid.append(RadLab(data_dir_radlab, instrument=args.rad_inst, date_start=exclusion_start, date_end=exclusion_end, rewind_minutes=args.delta_minutes))
            ## Unioning datasets
            if 'sdo' in args.input_datasets: 
            ...
            if 'sdo' in args.input_datasets:
                if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                    if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                        dataset_sequences_valid = Sequences([dataset_sdo_valid, dataset_goes_xrs_valid, dataset_rad_valid], delta_minutes=args.delta_minutes, sequence_length=training_sequence_length)
                    else: 
                        dataset_sequences_valid = Sequences([dataset_sdo_valid, dataset_rad_valid], delta_minutes=args.delta_minutes, sequence_length=training_sequence_length)
            else: ## No SDO data in the input
                if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                    if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                        ...
                        raise ValueError(f'Datasets not supported - input: {args.input_datasets} output: {args.output_datasets}')
            ...
            if args.test_event_id is not None:
                for event_id in args.test_event_id:
                    ...
                    date_exclusions.append((exclusion_start, exclusion_end))
                    if 'sdo' in args.input_datasets and args.solar_dataset == 'SDOMLlite':
                        datasets_sdo_test.append(SDOMLlite(data_dir_sdo, data_dir_sdo, date_start=exclusion_start, date_end=exclusion_end))
                    if 'sdo' in args.input_datasets and args.solar_dataset == 'SDOCore':
                        datasets_sdo_test.append(SDOCore(data_dir_sdo, date_start=exclusion_start, date_end=exclusion_end))
                    if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                    ...
                    if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                        datasets_rad_test.append(RadLab(data_dir_radlab, instrument=args.rad_inst, date_start=exclusion_start, date_end=exclusion_end, rewind_minutes=args.delta_minutes))
            ## Unioning datasets
            if 'sdo' in args.input_datasets:
            ...
            if 'sdo' in args.input_datasets:
                if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                    if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                        dataset_sequences_test = Sequences([dataset_sdo_test, dataset_goes_xrs_test, dataset_rad_test], delta_minutes=args.delta_minutes, sequence_length=training_sequence_length)
                    else: 
                        dataset_sequences_test = Sequences([dataset_sdo_test, dataset_rad_test], delta_minutes=args.delta_minutes, sequence_length=training_sequence_length)
            else: ## No SDO data in the input
                if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                    if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
            ...
            # Training set
            if 'sdo' in args.input_datasets and args.solar_dataset == 'SDOMLlite':
                datasets_sdo_train = SDOMLlite(data_dir_sdo, data_dir_sdo, date_start=args.date_start, date_end=args.date_end, date_exclusions=date_exclusions)
            elif 'sdo' in args.input_datasets and args.solar_dataset == 'SDOCore':
                datasets_sdo_train = SDOCore(data_dir_sdo, date_start=args.date_start, date_end=args.date_end, date_exclusions=date_exclusions)
            if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                datasets_goes_xrs_train = GOESXRS(data_dir_goes_xrs, date_start=args.date_start, date_end=args.date_end, date_exclusions=date_exclusions, rewind_minutes=args.delta_minutes)
            if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                datasets_rad_train = RadLab(data_dir_radlab, instrument=args.rad_inst, date_start=args.date_start, date_end=args.date_end, date_exclusions=date_exclusions, rewind_minutes=args.delta_minutes)
            ## Unioning datasets
            if 'sdo' in args.input_datasets:
                dataset_sdo_train = UnionDataset([datasets_sdo_train])
            if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                dataset_goes_xrs_train = UnionDataset([datasets_goes_xrs_train])
            if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                dataset_rad_train = UnionDataset([datasets_rad_train])
            ## Getting sequences
            if 'sdo' in args.input_datasets:
                if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                    if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                        dataset_sequences_train = Sequences([dataset_sdo_train, dataset_goes_xrs_train, dataset_rad_train], delta_minutes=args.delta_minutes, sequence_length=training_sequence_length)
                    else: 
                        dataset_sequences_train = Sequences([dataset_sdo_train, dataset_rad_train], delta_minutes=args.delta_minutes, sequence_length=training_sequence_length)
            else: ## No SDO data in the input
                if 'rad' in args.input_datasets or 'rad' in args.output_datasets:
                    if 'goes_xrs' in args.input_datasets or 'goes_xrs' in args.output_datasets:
                        dataset_sequences_train = Sequences([dataset_goes_xrs_train, dataset_rad_train], delta_minutes=args.delta_minutes, sequence_length=training_sequence_length)
                    else:
                        raise ValueError(f'Datasets not supported - input: {args.input_datasets} output: {args.output_datasets}')
            ...
            if len(model_files) > 0:
                ...
                if 'sdo' in args.input_datasets and args.solar_dataset == 'SDOMLlite':
                    if 'goes_xrs' in args.input_datasets and 'rad' in args.input_datasets:
                        if 'goes_xrs' in args.output_datasets and 'rad' in args.output_datasets:
                            model_data_dim_context = 2
                            model_data_dim_prediction = 2
                        elif 'goes_xrs' not in args.output_datasets and 'rad' in args.output_datasets:
                            model_data_dim_context = 2
                            model_data_dim_prediction = 1
                    elif 'goes_xrs' in args.input_datasets and 'rad' not in args.input_datasets:
                        if 'goes_xrs' in args.output_datasets and 'rad' in args.output_datasets:
                            model_data_dim_context = 1
                            model_data_dim_prediction = 2
                        elif 'goes_xrs' not in args.output_datasets and 'rad' in args.output_datasets:
                            model_data_dim_context = 1
                            model_data_dim_prediction = 1
                    elif 'goes_xrs' not in args.input_datasets and 'rad' in args.input_datasets:
                        if 'goes_xrs' in args.output_datasets and 'rad' in args.output_datasets:
                            model_data_dim_context = 1
                            model_data_dim_prediction = 2
                        elif 'goes_xrs' not in args.output_datasets and 'rad' in args.output_datasets:
                ...
                elif 'sdo' not in args.input_datasets and 'goes_xrs' in args.input_datasets and 'rad' in args.input_datasets:
                    if 'goes_xrs' in args.output_datasets and 'rad' in args.output_datasets:
                        model_data_dim_context = 2
                        model_data_dim_prediction = 2
                    elif 'goes_xrs' not in args.output_datasets and 'rad' in args.output_datasets:
            ...
            epoch_losses_file.write('epoch train_loss valid_loss\n')
            for epoch in range(epoch_start, args.epochs):
                ...
                with tqdm(total=len(train_loader)) as pbar:
                    for i, batch in enumerate(train_loader):
                        ...
                        if isinstance(model, RadRecurrentWithSDO):
                            (sdo, goesxrs, rad, _) = batch
                            batch_size = goesxrs.shape[0]
                            # print(sdo.shape, goesxrs.shape, rad.shape)
                            ...
                            rad = rad.unsqueeze(-1)
                            data = torch.cat([goesxrs, rad], dim=2)
                            ...
                            if 'goes_xrs' in args.input_datasets and 'rad' in args.input_datasets:
                                ...
                                context_data = rad[:, :model.context_window]
                            elif 'goes_xrs' in args.input_datasets and 'rad' not in args.input_datasets:
                                context_data = goesxrs[:, :model.context_window]
                            # print("context_sdo", context_sdo.shape, "context_data", context_data.shape)
                            # prediction window
                            if 'goes_xrs' in args.input_datasets and 'rad' in args.input_datasets:
                                input = data[:, model.context_window:-1] 
                            elif 'goes_xrs' not in args.input_datasets and 'rad' in args.input_datasets:
                                input = rad[:, model.context_window:-1]
                            elif 'goes_xrs' in args.input_datasets and 'rad' not in args.input_datasets:
                                input = goesxrs[:, model.context_window:-1]
                            if 'goes_xrs' in args.output_datasets and 'rad' in args.output_datasets:
                                target = data[:, model.context_window+1:]
                            elif 'goes_xrs' not in args.output_datasets and 'rad' in args.output_datasets:
                                target = rad[:, model.context_window+1:]
        ...
        if args.mode == 'test':
            print('\n*** Testing mode\n')
            assert args.model_file is not None, 'Model file not provided'
            ...
            if args.test_event_id is not None:
                ...
                for event_id in args.test_event_id:
                    ...
                    date_start = datetime.datetime.fromisoformat(date_start)
                    date_end = datetime.datetime.fromisoformat(date_end)
                    file_prefix = 'test-event-{}-{}-{}'.format(event_id,date_start.strftime('%Y%m%d%H%M'), date_end.strftime('%Y%m%d%H%M'))
                    title = 'Event: {} '.format(event_id)
            ...
            if args.train_event_id is not None:
                ...
                for event_id in args.train_event_id:
                    ...
                    date_start = datetime.datetime.fromisoformat(date_start)
                    date_end = datetime.datetime.fromisoformat(date_end)
                    file_prefix = 'train-event-{}-{}-{}'.format(event_id,date_start.strftime('%Y%m%d%H%M'), date_end.strftime('%Y%m%d%H%M'))
                    title = 'Event: {} '.format(event_id)
                    tests_to_run.append((date_start, date_end, file_prefix, title))
