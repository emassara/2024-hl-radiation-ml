            ...
            if len(model_files) > 0:
                ...
                if args.model_type == 'RadRecurrent':
                    ...
                    if not args.sdo_random_data and not args.xray_random_data and not args.rad_random_data:
                        ...
                        raise ValueError('Model w/o SDO not implemented')
                ...
                with tqdm(total=len(train_loader)) as pbar:
                    for i, batch in enumerate(train_loader):
                        ...
                        if isinstance(model, RadRecurrentWithSDO):
                            ...
                            if args.xray_random_data:
                                context_data = data[:, :model.context_window,1]
                                context_data = context_data.unsqueeze(-1)
                            elif args.rad_random_data:
                                context_data = data[:, :model.context_window,0]
                                context_data = context_data.unsqueeze(-1)
                            else:
